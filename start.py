...
from backend.config_loader import load_config, apply_config_to_env
...
def setup_environment():
    ...
    has_optimize_key = bool(os.getenv("OPENAI_OPTIMIZE_API_KEY"))
    has_summarize_key = bool(os.getenv("OPENAI_SUMMARIZE_API_KEY"))

    if not (has_default_key or has_optimize_key or has_summarize_key):
        print("⚠️  警告: 未设置OpenAI API Key环境变量")
        print("请设置环境变量: export OPENAI_API_KEY=your_api_key_here")
        print("或在 config.jsonc 中设置 openai.default/optimize/summarize.api_key")
        return False

    print("✅ 已设置OpenAI API Key")

    if not os.getenv("OPENAI_BASE_URL"):
        if not os.getenv("OPENAI_OPTIMIZE_BASE_URL") and not os.getenv("OPENAI_SUMMARIZE_BASE_URL"):
