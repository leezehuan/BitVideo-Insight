...
from config_loader import load_config, apply_config_to_env
...
async def process_video_task(task_id: str, url: str, summary_language: str):
    ...
    try:
        ...
        video_title = None
        try:
            subtitle_script, video_title, subtitle_lang = await video_processor.try_get_youtube_subtitle_transcript(
                url,
                preferred_language=summary_language,
            )
        except Exception as e:
            logger.info(f"尝试读取YouTube字幕失败，将回退到音频转录: {e}")
        ...
        # 优化转录文本：修正错别字，生成句级时间戳
        timestamped_script = await summarizer.optimize_transcript(raw_script)
        plain_script = summarizer.strip_sentence_timestamps(timestamped_script)
        ...
        screenshot_plan = await summarizer.request_screenshot_plan(timestamped_script)
        screenshots = []
        image_payloads = None
        annotated_script = plain_script
        ...
        if screenshot_plan:
            ...
            video_path, downloaded_title = await video_processor.download_video(url, TEMP_DIR)
            if not video_title:
                video_title = downloaded_title

            screenshot_folder_name = f"{safe_title}_{short_id}"
            screenshot_folder = SCREENSHOT_DIR / screenshot_folder_name
            ...
            screenshots = video_processor.extract_screenshots(
                Path(video_path),
                screenshot_plan,
                screenshot_folder,
            )
            ...
            if screenshots:
                image_url_prefix = f"/api/screenshot/{screenshot_folder_name}"
                annotated_script = summarizer.annotate_transcript_with_screenshots(
                    timestamped_script,
                    screenshots,
                    image_url_prefix=image_url_prefix,
                )
                image_payloads = summarizer.build_image_payloads(screenshots)
        
        # 为转录文本添加标题，并在结尾添加来源链接
        script_with_title = f"# {video_title}\n\n{annotated_script}\n\nsource: {url}\n"
        
        # 检查是否需要翻译
        detected_language = transcriber.get_detected_language(raw_script) or subtitle_lang
        ...
        if detected_language and translator.should_translate(detected_language, summary_language):
            ...
            # 翻译转录文本
            translation_content = await translator.translate_text(plain_script, summary_language, detected_language)
        ...
        task_result = {
            ...
            "screenshots": [shot.get("filename") for shot in screenshots] if screenshots else None,
            "short_id": short_id,
...
@app.get("/api/screenshot/{folder}/{filename}")
async def get_screenshot(folder: str, filename: str):
    ...
    try:
        ...
        if not (filename.lower().endswith(".jpg") or filename.lower().endswith(".jpeg") or filename.lower().endswith(".png")):
            raise HTTPException(status_code=400, detail="仅支持jpg/png截图")

        file_path = SCREENSHOT_DIR / folder / filename
        if not file_path.exists():
        ...
        raise HTTPException(status_code=500, detail="读取截图失败")
