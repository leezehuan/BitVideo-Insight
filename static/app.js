class VideoTranscriber {
    constructor() {
        ...
        this.translations = {
            en: {
                ...
                planning_screenshots: "Planning screenshots...",
                downloading_screenshot_video: "Downloading video for screenshots...",
                capturing_screenshots: "Capturing key frames...",
                translating_text: "Generating translation...",
            ...
            zh: {
                ...
                planning_screenshots: "正在规划截图...",
                downloading_screenshot_video: "正在下载截图源视频...",
                capturing_screenshots: "正在截取关键画面...",
                translating_text: "正在生成翻译...",
    ...
    updateProgressStage(progress, message) {
        ...
        if (message.includes('截图') || message.includes('screenshot')) {
            this.smartProgress.stage = 'screenshotting';
            this.smartProgress.target = 75;
        } else if (message.includes('解析') || message.includes('parsing')) {
        ...
        } else if (message.includes('翻译') || message.includes('translat')) {
            this.smartProgress.stage = 'translating';
            this.smartProgress.target = 85;
    ...
    calculateProgressIncrement() {
        ...
        const stageConfig = {
            ...
            'screenshotting': { speed: 0.25, maxTime: 60 }, // 截图阶段：1分钟内到75%
            'translating': { speed: 0.25, maxTime: 60 }, // 翻译阶段：1分钟内到85%
    ...
    getCurrentStageMessage() {
        const stageMessages = {
            ...
            'screenshotting': this.t('planning_screenshots'),
            'translating': this.t('translating_text'),
    ...
    updateProgressDisplay(progress, message) {
        ...
        if (message.includes('规划截图') || message.includes('screenshot plan') || message.includes('planning screenshots')) {
            translatedMessage = this.t('planning_screenshots');
        } else if (message.includes('下载截图源视频') || message.includes('screenshot video')) {
            translatedMessage = this.t('downloading_screenshot_video');
        } else if (message.includes('截取关键画面') || message.includes('capturing') || message.includes('Capture')) {
            translatedMessage = this.t('capturing_screenshots');
        } else if (message.includes('翻译') || message.includes('translation') || message.includes('Translat')) {
            translatedMessage = this.t('translating_text');
        } else if (message.includes('下载视频') || message.includes('downloading') || message.includes('Downloading')) {
